...
SAML.prototype.generateAuthorizeRequest = function (req, isPassive, isHttpPostBinding, callback) {
  ...
  .then(() => {
    ...
    var stringRequest = xmlbuilder.create(request, this.options.requestXmlbuilderOptions).end();
...
SAML.prototype.generateLogoutRequest = function (req) {
  ...
  return Q.ninvoke(this.cacheProvider, 'save', id, instant)
    .then(function() {
      return xmlbuilder.create(request, self.options.requestXmlbuilderOptions).end();
    });
};
...
SAML.prototype.validatePostResponse = function (container, callback) {
  ...
  Q.fcall(() => {
    ...
    inResponseTo = xpath(doc, "/*[contains(local-name(),'Response')]/@InResponseTo");
      ...
      .then(certs => {
        var hasValidQuerySignature = certs.some(cert => {
          return this.validateSignatureForRedirect(
            urlString, container.Signature, container.SigAlg, this.certToPEM(cert)
          );
        });
...
SAML.prototype.generateServiceProviderMetadata = function( decryptionCert, signingCert ) {
  ...
  if (this.options.supportedIdentifierFormats) {
    ...
    this.options.supportedIdentifierFormats.forEach(format => metadata.EntityDescriptor.SPSSODescriptor.NameIDFormat.push(format));
